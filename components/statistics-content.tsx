...
export default function StatisticsContent({ showHeader = true, className = "" }: StatisticsContentProps) {
    ...
    <div className={className}>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4 mb-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold dark:text-white">Portfolio Performance</h2>
          <div className="flex items-center space-x-2">
            <div className="flex space-x-1">
              <button
                className={`px-2 py-1 text-xs rounded ${chartType === "line" ? "bg-gray-200 dark:bg-gray-600" : ""}`}
                onClick={() => setChartType("line")}
              >
                Line
              </button>
              <button
                className={`px-2 py-1 text-xs rounded ${chartType === "bar" ? "bg-gray-200 dark:bg-gray-600" : ""}`}
                onClick={() => setChartType("bar")}
              >
                Bar
              </button>
              <button
                className={`px-2 py-1 text-xs rounded ${chartType === "candle" ? "bg-gray-200 dark:bg-gray-600" : ""}`}
            ...
            <div className="flex space-x-1">
              {["24H", "1W", "1M", "3M", "1Y", "ALL"].map((period) => (
                <button
                  key={period}
                  className={`px-2 py-1 text-xs rounded ${timeframe === period ? "bg-gray-200 dark:bg-gray-600" : ""}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4">
          <h2 className="text-lg font-bold dark:text-white mb-4">Top Gainers (24h)</h2>
              ...
              <tbody>
                {topGainers.map((coin, index) => (
                  <tr key={index} className="border-b dark:border-gray-700 last:border-0">
                    ...
                    <td className="py-3 text-right dark:text-white">{coin.price}</td>
        ...
        {/* Top Losers */}
        <div className="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4">
          <h2 className="text-lg font-bold dark:text-white mb-4">Top Losers (24h)</h2>
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className="text-xs text-gray-500 border-b dark:border-gray-700">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold dark:text-white">Trading Volume</h2>
          <div className="flex items-center space-x-2">
            <button className="p-1 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700 border dark:border-gray-700">
        ...
        <div className="h-60 w-full">
          ...
          <svg className="w-full h-full" viewBox="0 0 800 200">
            ...
            {/* Trading volume bars */}
            {performanceData.map((data, i) => {
              const x = 50 + i * (700 / performanceData.length)

...
- **UI/UX Decisions**:
  ...
  - **商户管理功能**：
    ...
    - **账户余额显示**：垂直堆叠显示三种余额，点击可查看详情
      - 可用余额：主要余额（黑色/白色）
      - 代付金余额：用于代付的专用余额（蓝色）
      - 冻结余额：被冻结的金额（红色/灰色）
    - **账户余额详情弹窗**：点击账户余额打开详情对话框
      - **卡片网格布局**：2列网格布局（大屏）或1列布局（小屏），一次性显示所有5种币种
      - **币种卡片**：每个币种独立卡片，包含渐变背景标题栏显示币种符号和名称
      - **双余额展示**：每个卡片显示法币余额（可用余额）和代付金余额
      - **大字号展示**：法币余额和代付金余额使用2xl字体醒目显示，总余额使用xl字体
      - **币种符号**：自动根据币种显示对应符号（¥/$/ €/£/₮）
      - **总余额统计**：每个卡片底部显示该币种的总余额（法币余额+代付金余额）
      - **颜色区分**：法币余额黑色/白色，代付金余额蓝色，总余额黑色/白色加粗
      - **交互效果**：卡片hover时阴影加深，提供视觉反馈

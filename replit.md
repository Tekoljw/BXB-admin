...
- **UI/UX Decisions**:
  ...
  - **商户管理功能**：
    ...
    - **账户余额详情弹窗**：点击账户余额打开详情对话框
      - **币种页签**：使用Tabs组件按币种（CNY、USD、USDT、EUR、GBP）分页签展示
      - **双余额展示**：每个币种显示法币余额（可用余额）和代付金余额
      - **大字号展示**：法币余额和代付金余额使用大字号（3xl）醒目显示
      - **币种符号**：自动根据币种显示对应符号（¥/$/ €/£/USDT）
      - **总余额统计**：自动计算并显示法币余额+代付金余额的总和
      - **颜色区分**：法币余额黑色/白色，代付金余额蓝色

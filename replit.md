...
- **UI/UX Decisions**:
  ...
  - **代理商管理功能**：
    ...
    - 展示代理商基本信息（代理商ID、代理商名称、商户ID、UserID、邮箱、电话、佣金数据、商户数量、状态）
    - 代理商本身即是商户，也是用户，因此同时显示商户ID和UserID
    - **状态管理**：支持启用、冻结、停用三种状态
      - 绿色徽章：启用状态
      - 黄色徽章：冻结状态
      - 灰色徽章：停用状态
    - **冻结功能**：点击冻结按钮，弹出对话框要求选择币种和输入冻结金额
      - 支持币种：CNY、USD、USDT、EUR、GBP
      - 冻结后状态自动变为"冻结"，解冻后状态变为"启用"
      - 按钮图标动态显示（锁图标/解锁图标）
  ...
  - **商户管理功能**：
    ...
    - **账户余额显示**：在商户列表中显示法币余额和代付金余额
      - 法币余额：商户的法币资产余额
      - 代付金余额：商户的代付金资产余额
      - 查看详情按钮：点击可查看各币种的详细余额明细
    - **余额详情对话框**：按币种显示各个币种的法币余额和代付金余额
      - 支持币种：CNY、USD、USDT、EUR、GBP等
      - 表格显示：币种、法币余额（绿色）、代付金余额（蓝色）
      - 总计行：显示所有币种的法币余额总计和代付金余额总计

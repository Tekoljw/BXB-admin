...
### 2025-11-27: U卡供应商页面余额和币种优化
- **供应商资金池显示**: 在供应商名称后面显示资金池信息
  - 有资金池：显示蓝色渐变圆角标签，包含"资金池"文字、余额数字和黄色"USDT"标识
  - 无资金池：显示灰色"无资金池"标签
  - 供应商详情弹窗中也显示资金池状态和余额
- **供应商USDT余额显示**: 在供应商名称旁边显眼展示存入的总USDT余额
  - 采用绿色渐变圆角标签设计，包含"存入"文字、余额数字和黄色"USDT"标识
  - 供应商详情弹窗顶部也显示大号的存入总额
- **卡种单一币种**: 每个卡种只支持一个币种（之前是多币种）
  - 币种标签显示在卡片头部右侧，采用彩色背景（USD绿色、EUR蓝色、GBP紫色等）
- **卡种余额显眼展示**: 在每个卡种卡片中添加醒目的余额显示区域
  - 采用琥珀色/金色渐变背景，突出显示余额数字和币种
  - 余额数字为大号粗体，币种标签紧随其后
- **号段弹窗改为右侧滑出**: 点击号段按钮时，弹窗从右侧滑出（Sheet组件），与其他详情弹窗风格一致
  - 号段采用卡片式布局，每个号段独立一张卡片
  - 显示号段名称、创建时间、卡号范围、使用量进度条和状态标签

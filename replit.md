...
- **BePay管理模块**: 包含9个子页面
  ...
  - **接口管理**: 管理支付接口
  - **通道管理**: 管理支付通道
    - **LOGO显示**：每个支付通道显示其品牌LOGO图标
    - **外显名称**：支持设置用户可见的通道显示名称（例如："支付宝扫码支付"）
    - **三档阶梯费率**：根据交易金额范围自动匹配不同档位的手续费率
      - 第1档（小额交易）：0 - 10,000
      - 第2档（中额交易）：10,000 - 100,000
      - 第3档（大额交易）：100,000 及以上，鼓励大额交易
      - 每档包含：代收费率、最低代收费、代付费率、最低代付费四个参数
    - **费率详情查看**：点击"查看三档费率"按钮，以对话框形式展示所有费率档位的详细信息
    - **添加/编辑功能**：支持设置通道LOGO、外显名称和三档阶梯费率的完整配置
    - **表格简化**：将原代收费率和代付费率列合并为"阶梯费率"列，通过按钮查看详情
  - **商户管理**: 管理商户信息
  - **代理商管理**: 管理代理商及佣金比例
...
- **UI/UX Decisions**:
  ...
  - **商户管理功能**：
    ...
    - **状态开关控制**：使用开关按钮控制商户状态
      ...
      - 开关关闭（灰色）：冻结状态
      - 禁用状态：开关不可操作，显示为灰色
      - 状态文字：开关旁边显示当前状态（"正常"/"冻结"/"禁用"）
    - **账户余额显示**：垂直堆叠显示两种余额，点击可查看详情
      - 可用余额：主要余额（黑色/白色）
      - 代付金余额：用于代付的专用余额（蓝色）
    - **账户余额详情弹窗**：点击账户余额打开详情对话框（紧凑的列表形式）
      - **弹窗尺寸**：max-w-3xl宽度，max-h-75vh高度，保持紧凑
      - **表格布局**：使用表格形式一次性显示所有5种币种余额
      - **表头**：币种、法币余额、代付金余额、总余额
      - **币种列**：显示币种符号（¥/$/ €/£/₮）+ 币种名称
      - **法币余额列**：右对齐，黑色/白色字体
      - **代付金余额列**：右对齐，蓝色字体标识
      - **总余额列**：右对齐，加粗显示（法币余额+代付金余额）
      - **斑马纹**：交替行背景色，提高可读性
      - **响应式**：支持水平滚动，适配小屏设备

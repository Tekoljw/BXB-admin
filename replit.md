...
- **运营模块**: 包含27个子页面，采用**两级菜单分类结构**，分为4个功能分组：【用户管理】（注册白名单、KYC限额管理、现货费率管理）、【运营配置】（WEB注册页广告、国家/地区管理、2FA场景配置、提示语配置、金刚区管理、官方渠道配置、广告位管理、榜单配置、首页弹窗、Push通知、Banner管理、站内配置）、【内容管理】（邮件内容、邮件发送、荣誉管理、日程安排管理、语音发料、消息发送、创建邮件）、【运营活动】（抵扣金明细、体验金明细、用户抵扣金、用户体验金、卡券配置）。左侧边栏显示分组标题（使用【】符号标识）和对应的二级菜单项。
- **功能优化**:

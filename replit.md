...
- **审核模块**: 包含25个子页面，采用**两级可折叠菜单分类结构**，分为4个功能分组：
  - 用户管理：用户管理、黑名单管理、已删除账户、用户行为、API审核、API管理、短信日志、邮箱日志、重置2FA审核、语音日志
  - 特殊账户管理：账户类型管理、特殊账户管理
  - 实名认证：KYC审核、审核历史
  - 资金管理：用户资产、资金记录、充值速进、提现速进、小额快速提现、资金冻结、充值记录、热钱包余额、提现记录、充值入账申请、提现审核
  - 左侧边栏显示分组标题和对应的二级菜单项，支持分组折叠/展开。点击顶部导航"审核"按钮时，左侧边栏显示所有4个分组的完整菜单，默认全部折叠。样式与运营模块统一。
- **运营模块**: 包含42个子页面，采用**两级可折叠菜单分类结构**，分为6个功能分组：
  - 用户管理：注册白名单、KYC限额管理、现货费率管理
  - 运营配置：WEB注册页广告、国家/地区管理、2FA场景配置、提示语配置、金刚区管理、官方渠道配置、广告位管理、榜单配置、首页弹窗、Push通知、Banner管理、站内配置、行情配置
  - 内容管理：邮件内容、邮件发送、荣誉管理、日程安排管理、语音发料、消息发送、创建邮件
  - 运营活动：抵扣金明细、体验金明细、用户抵扣金、用户体验金、卡券配置
  - 运营报表：运营报表、运营活动、审核监控、资金流水、订单监控、留存分析、风控预警（路径保持为/admin/operations/*）
  - 运维管理：权限管理、角色管理、用户管理、审核配置、APP管理、运维管理、维护计划配置、维护白名单管理（路径保持为/admin/system/*）
  - 左侧边栏显示分组标题和对应的二级菜单项，支持分组折叠/展开。点击顶部导航"运营"按钮时，左侧边栏显示所有6个分组的完整菜单，默认全部折叠。
- **功能优化**:

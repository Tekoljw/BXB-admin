...
- **功能优化**:
    ...
    - **法币模块**: 包含经营报表（顶部8个统计卡片展示用户统计、今日/昨日业务、下发统计、资金统计、今日/昨日/本月利润分解，支持时间范围筛选：今天/昨天/最近7天/最近30天/自定义，支持CSV导出，所有金额使用USDT单位）、币种管理、供应商管理、接口管理、通道管理、法币用户管理、代理商管理、代收订单、代付订单、兑换订单等子模块。经营报表页面包含每日/每月报表表格视图，可展开查看利润前10支付渠道和利润前10用户详情。**注意**：法币资产管理已移至财务模块。

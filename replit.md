...
- **功能优化**:
    ...
    - **财务模块（orders）**: 包含财务经营报表（路径：`/admin/orders/business-overview`，展示各业务模块收入、成本、利润汇总）、Crypto资产管理（路径：`/admin/orders/crypto-assets`）和法币资产管理（路径：`/admin/orders/fiat-assets`）三个子页面。点击顶部财务按钮默认打开财务经营报表。

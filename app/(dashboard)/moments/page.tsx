...
const MarketPageContent = () => {
  ...
  const getFilteredData = () => {
    ...
    if (activeMainTab === "自选") {
      filtered = marketData.filter(crypto => 
        favorites.includes(`${crypto.symbol}/${crypto.pair}`)
      )
    } else if (activeMainTab === "热门") {
      filtered = marketData.slice(0, 6)
    } else if (activeMainTab === "涨幅榜") {
      filtered = marketData.filter(crypto => crypto.isPositive).sort((a, b) => 
        parseFloat(b.change) - parseFloat(a.change)
      )
    } else if (activeMainTab === "跌幅榜") {
      filtered = marketData.filter(crypto => !crypto.isPositive).sort((a, b) => 
        parseFloat(a.change) - parseFloat(b.change)
      )
    } else if (activeMainTab === "新币榜") {
      filtered = marketData.slice(2, 5)
    } else if (activeMainTab === "成交额榜") {
      filtered = marketData.sort((a, b) => 
        parseFloat(b.volume.replace(/[,K]/g, '')) - parseFloat(a.volume.replace(/[,K]/g, ''))
      )
    }
  ...
  // 应用搜索过滤器
  const filteredData = getFilteredData().filter(crypto =>
